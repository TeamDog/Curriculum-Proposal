...
class SeedDMS_Bootstrap_Style extends SeedDMS_View_Common {
	...
	function htmlStartPage($title="", $bodyClass="", $base="") { /* {{{ */
		...
		//BEGIN jtg's mod
		echo '<link href="../custom/styles/globalNav.css" rel="stylesheet">'."\n";
		//END jtg's mod
...
//		$content .= "    <li><a href=\"../op/op.ClearClipboard.php?refferer=".urlencode($this->params['refferer'])."\">".getMLText("clear_clipboard")."</a><a class=\"ajax-click\" data-href=\"../op/op.Ajax.php\" data-param1=\"command=clearclipboard\">kkk</a> </li>\n";
	...
	function globalNavigation($folder=null) { /* {{{ */
		$dms = $this->params['dms'];
		//BEGIN jtg's mod
		// WAS: echo "<div class=\"navbar navbar-inverse navbar-fixed-top\">\n";
		echo "<div class=\"navbar navbar-inverse\">\n";
		echo file_get_contents("../custom/globalNav.html");
		//END jtg's mod
		echo " <div class=\"navbar-inner\">\n";
		echo "  <div class=\"container-fluid\">\n";
...
//			}
	...
	private function myDocumentsNavigationBar() { /* {{{ */
		...
		echo "<id=\"first\"><a href=\"../out/out.MyDocuments.php?inProcess=1\" class=\"brand\">Dashboard</a>\n";
		echo "<div class=\"nav-collapse col2\">\n";
		echo "<ul class=\"nav\">\n";
		
		//echo "<li><a href=\"../out/out.MyDocuments.php?inProcess=1\">".getMLText("documents_in_process")."</a></li>\n"; 
		echo "<li><a href=\"../out/out.MyDocuments.php?inProcess=1\">My Queue</a></li>\n";
		//echo "<li><a href=\"../out/out.MyDocuments.php\">".getMLText("all_documents")."</a></li>\n";
		echo "<li><a href=\"../out/out.MyDocuments.php\">My Proposals</a></li>\n";
...
<?php
	...
	function documentListRow($document, $previewer, $skipcont=false, $version=0) { /* {{{ */
		...
		//PAT MOD Start getting attribute and assigning value to $processType
		$attributes = array();
		...
		if($latestContent) {
			...
			if($status["status"] == S_IN_WORKFLOW && $workflowmode == 'advanced') {
				...
				//jtg mod, resolving a bug. TODO FIND CAUSE
				if((strcmp(gettype($workflow),"object") == 0) && (strcmp(gettype($workflowstate),"object") == 0)) {
					$content .= '<span title="'.getOverallStatusText($status["status"]).': '.$workflow->getName().'">'.$workflowstate->getName().'</span>';
				} else {
					$content .= '<span title="'.getOverallStatusText($status["status"]).': '.'">'.'</span>';
				}
...
//				$content .= "<td>".$version."</td>";
				//PAT MOD fill with processType info
				$content .= "<td>".$processType."</td>";
			$content .= "<td>";
			...
			if($document->getAccessMode($user) >= M_WRITE) {
