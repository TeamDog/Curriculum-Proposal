...
class SeedDMS_View_MyDocuments extends SeedDMS_Bootstrap_Style {
	...
	function show() { /* {{{ */
		...
		$this->htmlStartPage("Dashboard");
		$this->globalNavigation();
		$this->contentStart();
		$this->pageNavigation("My Proposals", "my_documents");
		...
		if ($showInProcess){
			...
			if($workflowmode == 'traditional' || $workflowmode == 'traditional_only_approval') {
				...
				if (strlen($docCSV)>0) {
					...
					$this->contentHeading("My Proposal Queue");
					$this->contentContainerStart();
					$printheader=true;
					$iRev = array();
					$dList = array();
					foreach ($workflowStatus["u"] as $st) {
						...
						if ( isset($docIdx[$st["document"]][$st["version"]]) && !in_array($st["document"], $dList) ) {
							...
							//PAT MOD Reordered item display to be in line with headers
							print "</a></td>";
					...
					print "No proposals require your attention.";
				...
				$this->contentHeading("Your Open Proposals");
				$this->contentContainerStart();
				if (count($resArr)>0) {
...
//						print "<td>".getOverallStatusText($res["status"])."</td>";
			...
			*/
			...
			$this->contentHeading("All Proposals");
			$this->contentContainerStart();
