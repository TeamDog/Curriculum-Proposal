...
?>
	...
	var showProperFields = function(){
		...
		currProposalType = $('#process-type-select').find(":selected").text();
		if(currProposalType != null && currProposalType !== '') {
			fieldMap[currProposalType].forEach(function(element){
					$("#attr" + element).show();
				});
	...
	$("#process-type-select").change(showProperFields);
...
?>
	...
	<?php
		...
		<tr id="attrProcessType">
			<td>Process Type</td>
			<td>
				<select id="process-type-select" name="process_select" required>
					<?php
