...
include("../custom/class.RowanCoverLetterGenerator.php");
session_start();
...
<?php
	...
	function show() { /* {{{ */
		...
		/******
			Start test for Rowan Cover Letter Generator
		******/
	
		echo "<hr />";
		echo "<div style='position: relative; height: 30px;'>";
		echo "<h5 style='font: Helvetica Neue 21px; position: absolute; left: 0px;'>Printing options: </h5>";
		echo "<a href=../out/out.CoverLetter.php?documentid=".$document->getID()." style='position: absolute; left: 200px; font: 18px Helvetica; text-decoration: none; background-color: #EEEEEE; color: #333333; padding: 6px 8px 6px 8px; border-radius: 5px; border: 1px solid #AAAAAA;'>Print Cover Letter</a>";
		echo "</div>";
		echo "<hr />";

		
		/******
			End test for Rowan Cover Letter Generator
		******/
...
<?php
			...
			$this->contentContainerEnd();
			echo "</div>";
			if(false) { //originally filled  with $user_is_involved || $user->isAdmin()
				echo "<div class=\"span6\">";
?>
