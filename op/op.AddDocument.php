...
$college = trim($_POST["college_select"]);
$processType = trim($_POST["process_select"]);
//$processType = null;
...
foreach($attributes as $attrdefid=>$attribute) {
	...
	if($attribute) {
		...
		//bad hack to get proposal status stored as an attribute
		/*if($attrdef->getID() == 3) {
			//store value of processType into attributes array
			echo "found processtype";
			$attributes[$attrdefid] = $processType;
		} elseif(strcmp($attrdef->getName(), "Department") == 0){
			echo "found Department";
			$attributes[$attrdefid] = $department;
		} elseif(strcmp($attrdef->getName(), "College") == 0){
			echo "found College";
			$attributes[$attrdefid] = $college;
		}*/
...
//manually inject attribute values without validation
$attributes['3'] = $processType;

$collegeGroup = $dms->getGroup($college);
...
//START workflow generation call HERE

$workflow = RowanWorkflowCreator::createWorkflow($owner, $processType, $department, $college,$dms);
  //$workflow = RowanWorkflowFactory::createWorkflow($owner, $proposalType, $department, $college);

//END workflow generation call

//OLD WORKFLOW GENERATION OR SETTING CODE
/*
